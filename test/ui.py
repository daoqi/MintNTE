...
class ParticleOverlay(QWidget):
    ...
    def draw_diamond(self, painter, size, angle):
        ...
        qpts = [QPointF(x, y) for x, y in rotated]
...
class NeonSelectButton(QPushButton):
    ...
    def paintEvent(self, event):
        ...
        painter.translate(-cx, -cy)
        super().paintEvent(event)          # 绘制文字和背景
        ...
        if self._selected:
            ...
            if rect.width() > 0 and rect.height() > 0:
                ...
                gradient = QLinearGradient(
                    gro_rect.left() + self.offset * gro_rect.width(),
                    0,
                    gro_rect.left() + (self.offset + 0.15) * gro_rect.width(),
                    0
                )
...
class BasePage(QWidget):
    def __init__(self, title, color, bg_color, log_bg, log_text):
        ...
        layout = QVBoxLayout()
        lbl = QLabel(title)
        lbl.setStyleSheet(f"color: {color}; font-size:{dp(22)}px; font-weight:bold;")
        layout.addWidget(lbl)
        log = QTextEdit()
        log.setReadOnly(True)
        log.setStyleSheet(f"background-color: {log_bg}; color: {log_text}; border:1px solid {color}; border-radius:{dp(8)}px;")
