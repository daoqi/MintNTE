...
class NeonSelectButton(QPushButton):
    def __init__(self, text, neon_color, parent=None):
        ...
        self.ripple_alpha = 0; self.ripple_radius = 0
        self._scale_factor = 1.0
        self.shock_alpha = 0; self.shock_radius = 0

        self.setFixedSize(120, 50); self.setCheckable(True)
        self.toggled.connect(self._on_toggled); self.clicked.connect(self._start_click_anim)
        self._update_style()
        self.glow = QGraphicsDropShadowEffect(self)
        self.glow.setColor(self.neon_color); self.glow.setOffset(0,0); self.glow.setBlurRadius(28)
        self.setGraphicsEffect(self.glow)

        self.ripple_timer = QTimer(self); self.ripple_timer.timeout.connect(self._ripple_animate)
    ...
    def _start_click_anim(self):
        self.ripple_alpha=255; self.ripple_radius=0
        if self.ripple_timer.isActive(): self.ripple_timer.stop()
        self.ripple_timer.start(16)
        self.shock_alpha=255; self.shock_radius=0
        self.scale_anim.stop(); self.scale_anim.start()

    def _update_style(self):
        color_name=self.neon_color.name()
        self.setStyleSheet(f"""
            QPushButton {{ background-color:#1a1a2e; color:{color_name}; border:none; border-radius:18px;
                          font-size:15px; font-weight:bold; padding:0px 12px; }}
            QPushButton:hover {{ background-color:{color_name}; color:#0a0a18; }}
        """)
    def set_neon_color(self, color_str):
        self.neon_color=QColor(color_str); self._update_style(); self.glow.setColor(self.neon_color); self.update()
    def _on_toggled(self, checked):
        self._selected = checked
        self.update()
    def _ripple_animate(self):
        self.ripple_radius += 4; self.ripple_alpha -= 10
        if self.ripple_alpha <=0: self.ripple_timer.stop()
        self.shock_radius += 6; self.shock_alpha -= 12
    ...
    def paintEvent(self, event):
        ...
        if self._selected:
            ...
            painter.translate(cx,cy); painter.scale(self._scale_factor, self._scale_factor); painter.translate(-cx,-cy)
            margin = 8; rect = self.rect().adjusted(margin,margin,-margin,-margin)
            if rect.width()>0 and rect.height()>0:
                radius = rect.height()/2
                painter.setPen(QPen(QColor(0,0,0), 5)); painter.setBrush(Qt.NoBrush)
...
class BasePage(QWidget):
    def __init__(self, title, color, bg_color):
        super().__init__()
        ...
        # 不再放置日志框，使右侧面板内容高度始终可控
        self.setLayout(layout)
class FishingPage(BasePage):
    def __init__(self, theme): super().__init__("🎣 自动钓鱼", theme["page_title_color"], theme["panel_bg"])
class MissionPage(BasePage):
    def __init__(self, theme): super().__init__("📋 自动任务", theme["page_title_color"], theme["panel_bg"])
class CombatPage(BasePage):
    def __init__(self, theme): super().__init__("⚔️ 自动战斗", theme["page_title_color"], theme["panel_bg"])
class ExchangePage(BasePage):
    def __init__(self, theme): super().__init__("💱 自动兑换", theme["page_title_color"], theme["panel_bg"])
class PinkClawPage(BasePage):
    def __init__(self, theme): super().__init__("🦊 粉爪助手", theme["page_title_color"], theme["panel_bg"])
class DrivingPage(BasePage):
    def __init__(self, theme): super().__init__("🚗 自动驾驶", theme["page_title_color"], theme["panel_bg"])
...
class MainWindow(QMainWindow):
    def __init__(self, config):
        ...
        self.log_handler = QTextEditHandler(); self.log_handler.new_log.connect(self.append_log)
    ...
    def init_ui(self):
        ...
        self.log_checkbox = QCheckBox("运行日志"); self.log_checkbox.setChecked(False)
        self.log_checkbox.stateChanged.connect(self.toggle_log_panel)
        ...
        right_layout.addWidget(self.stack, 1)  # stack 占用全部可用空间
        # 日志面板固定在底部，且不参与伸缩
        self.log_text = QTextEdit(); self.log_text.setReadOnly(True); self.log_text.setMaximumHeight(100)
        self.log_text.setStyleSheet("background-color: #12122a; color: #aaaaff; border:1px solid #555;")
        self.log_text.hide()
        right_layout.addWidget(self.log_text, 0)  # stretch = 0，不抢空间
    ...
    def toggle_log_panel(self, state):
        if state == Qt.Checked: self.log_text.show()
        else: self.log_text.hide()
    ...
    def append_log(self, msg):
        ...
        if doc.blockCount() > 150:
            ...
            if len(lines) > 150: self.log_text.setPlainText('\n'.join(lines[-150:]))
